...
def farthest_point_sample(xyz, npoint):
    ...
    for i in range(npoint):
        ...
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 2) # LEOX3 # when: l0_xyz = xyz[:,:2,:] # LEOX3 
...
class PointNetSetAbstractionMsg(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        ...
        for i in range(len(mlp_list)):
            convs = nn.ModuleList()
            bns = nn.ModuleList()
            #last_channel = in_channel + 4   # prev 3, now 4 LEOX2
            last_channel = in_channel + 2   # LEOX3
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            grouped_xyz = index_points(xyz, group_idx) #error here (245)? LEOX

#! /usr/bin/env python3
'''
For calculating mAP and F1 score
'''
...
def print_eval_stats(metrics_output, class_names, verbose):
    ...
    if metrics_output is not None:
        ...
        print(f"---- mAP {map_total:.5f} ----")
        print(f"---- F1 {f1_total:.5f} ----")
...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose):
    ...
    # get snippet here!!
    # a batch, snippets are wraped in a tuple
    for snip, imgs, targets in tqdm.tqdm(dataloader, desc="Validating"):
        # Extract labels
        labels += targets[:, 1].tolist()
        # Rescale target
        targets[:, 2:] = xywh2xyxy(targets[:, 2:])
        # absolute value in pixel coordinate <x1,y1,x2,y2>
        targets[:, 2:] *= img_size
...
def run():
    ...
    parser.add_argument("-m", "--model", type=str, default="yolo/config/yolov3.cfg",
                        help="Path to model definition file (.cfg)")
    parser.add_argument("-w", "--weights", type=str, default="yolo/checkpoints/yolov3_ckpt_640.pth",
                        help="Path to weights or checkpoint file (.weights or .pth)")
    parser.add_argument("-d", "--data", type=str, default="yolo/config/custom.data",
    ...
    test_snip_path = data_config["test_snip"]
